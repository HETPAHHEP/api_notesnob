...
class TestWorkflow:
    ...
    def test_workflow(self):
        notesnob_workflow_basename = 'notesnob_workflow'

        yaml = f'{notesnob_workflow_basename}.yaml'
        is_yaml = yaml in os.listdir(root_dir)

        yml = f'{notesnob_workflow_basename}.yml'
        ...
        if is_yaml and is_yml:
            assert False, (
                f'В каталоге {root_dir} не должно быть двух файлов {notesnob_workflow_basename} '
        ...
        assert (
                re.search(r'on:\s*push:\s*branches:\s*-\smaster', notesnob) or
                'on: [push]' in notesnob or
                'on: push' in notesnob
        ), f'Проверьте, что добавили действие при пуше в файл {filename}'
        assert 'pytest' in notesnob, f'Проверьте, что добавили pytest в файл {filename}'
        assert 'appleboy/ssh-action' in notesnob, f'Проверьте, что добавили деплой в файл {filename}'
        assert 'appleboy/telegram-action' in notesnob, (
